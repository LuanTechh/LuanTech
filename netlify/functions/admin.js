...
export const handler = async (event) => {
  ...
  try {
    if (action === "login" && event.httpMethod === "POST") {
      if (!PASSWORD || !SECRET) {
        return json(500, {
          error: "Faltan ADMIN_PASSWORD o TOKEN_SECRET en Netlify."
        });
      }
    ...
    if (!validToken(bearer(event))) {
      return json(401, { error: "Sesión no autorizada o vencida." });
    }

    const store = getLuanTechStore();

    if (action === "check") {
      return json(200, { ok: true, expiresIn: TOKEN_TTL });
    }
    ...
    if (action === "save" && event.httpMethod === "POST") {
      ...
      if (!p.name || !p.price || !p.category || !p.image) {
        return json(400, {
          error: "Completa nombre, precio, categoría e imagen."
        });
      }

      if (
        !/^data:image\/(png|jpeg|jpg|webp|gif);base64,/i.test(p.image) &&
        !/^https?:\/\//i.test(p.image)
      ) {
        return json(400, {
          error: "La imagen debe ser una URL HTTPS o una imagen válida."
        });
      }
    ...
    if (action === "import" && event.httpMethod === "POST") {
      ...
      if (!Array.isArray(incoming)) {
        return json(400, {
          error: "El archivo JSON debe contener una lista de productos."
        });
      }
    ...
    return json(500, {
      error: "No se pudo completar la operación en Netlify.",
      code: "FUNCTION_ERROR"
    });
  }
};
